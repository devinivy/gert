...
internals.Graph.prototype.getVertices = function (vertices /* or a label */, onlyIds) {
    ...
    for (var i = 0; i < vertices.length; i++) {
        ...
        if (this.vertexExists(v)) {
            keyedVertices[v] = onlyIds ? true : this.getVertex(v);
        }
    }

    return onlyIds ? Object.keys(keyedVertices) : keyedVertices;
};
...
internals.Graph.prototype.getEdges = function (pairs /* or a label */, onlyPairs) {
    ...
    var alreadyHasPair = function (u, v) {

        if (self.directed) {
            return pairsObj[u] && pairsObj[u][v];
        }

        return (pairsObj[u] && pairsObj[u][v]) || (pairsObj[v] && pairsObj[v][u]);
    };
    ...
    for (var i = 0; i < pairs.length; i++) {
        u = pairs[i][0];
        v = pairs[i][1];
        if (this.edgeExists(u, v) && !alreadyHasPair(u, v)) {
            pairsObj[u] = pairsObj[u] || {};
            pairsObj[u][v] = onlyPairs ? [u, v] : this.getEdge(u, v);
        }
    }

    return Utilities.pairsObjToArray(pairsObj, true);
};
...
internals.Graph.prototype.order = function () {
...
internals.Graph.prototype.traverse = function (starting, record) {
