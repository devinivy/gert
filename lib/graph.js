...
internals.Graph.prototype.getEdge = function (u, v) {
    ...
    var edge = (this._edges[u] && this._edges[u][v]) || null;
...
internals.Graph.prototype.subgraph = function (subset) {
    ...
    subVertices = subset.vertices || [];
    subEdges = subset.edges || [];
    ...
    var nonEdges = subEdges.filter(function (pair) {
        return !self._edges[pair[0]] || !self._edges[pair[0]][pair[1]];
    });
...
internals.Graph.prototype.adjacencyMatrix = function () {
    ...
    for (var i = 0; i < vertexList.length; i++) {
        ...
        for (var j = 0; j < vertexList.length; j++) {
            ...
            if (this._edges[u] && this.edges[u][v]) {
                row.push(1);
            } else {
                row.push(0);
            }
