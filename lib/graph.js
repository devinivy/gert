...
internals.Graph.prototype.union = function (graph) {
    ...
    // For traversing
    var edgeObj = this.digraph ? edgeInfo : Formatting.halvePairsObj(edgeInfo);
...
internals.Graph.prototype.intersection = function (graph) {
    ...
    for (var i = 0; i < vertexList.length; i++) {
        v = vertexList[i];
        vertices[v] = {
            labels: Hoek.intersect(ourVertices[v].labels, theirVertices[v].labels),
    ...
    // For traversing
    var edgeObj = this.digraph ? ourEdgeInfo : Formatting.halvePairsObj(ourEdgeInfo);
    ...
    for (var j = 0; j < us.length; j++) {
        ...
        for (var k = 0; k < ws.length; k++) {
            ...
            if (graph._lookupEdge(u, w)) {
                edges.push({
                    pair: [u, w],
                    labels: Hoek.intersect(ourEdgeInfo[u][w].labels, theirEdgeInfo[u][w].labels),
...
internals.Graph.prototype.join = function (graph, bothWays) {
